...
for time = timevec
	...
	val = [ val xin(poi)];
	CNerr = [CNerr xin'*M*xin+xin'*A*xin];
end
figure(2)
plot(timevec, CNerr)
title('Energy Error Over Time, Crank-Nicholson')
xlabel('Time')
ylabel('Energy Error')

figure(3)
plot(timevec, val)
title('Value at node (1,-1), Crank-Nicholson Euler')
